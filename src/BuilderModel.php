...
class BuilderModel extends Model
{
    ...
    foreach ($this->fieldsData as $fieldName => $fieldOptions) {
        if (isset($fieldOptions['link']) && isset($fieldOptions['as'])) {
          if ($this->isFetchable($fieldOptions['as'])) {
            $this->{$fieldOptions['as']} = $this->getLinked(self::getPhpName($fieldOptions['link'][0]), $fieldOptions['link'][1], $this->{$fieldName}, $this->getFetchable($fieldOptions['as']));
          }
        } else if (isset($fieldOptions['list'])) {
          /*
          $content .= "      if (\$this->isFetchable('" . $fieldName . "')) {\n";
          $content .= "        \$this->" . $fieldName . " = \$this->getLinked" . self::getPhpName($fieldName) . "List(\$this->" . $primaryKey . ", \$this->getFetchable('" . $fieldName . "'));\n";
          $content .= "      }\n";
          */
        }
      }
  ...
  private function getLinked($class, $field, $id, $fetchable) {
    static $count = 0;
    $count++;
    var_dump($class, $field, $id, $fetchable, $this->fetchable);
    echo("count = $count\n\n");
    if ($count > 10) die();
    if ($fetchable === false) {
      return $this;
    }
    $entity = $class::where($field, '=', $id);
    if ($fetchable != false) {
